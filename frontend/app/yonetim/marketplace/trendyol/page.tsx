...
export default function TrendyolPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const res = await api.get<SyncLog[]>(
        "/admin/marketplace/sync-logs?marketplace=trendyol"
      );
      setLogs(res.data ?? []);
    } catch (err) {
  ...
  async function triggerSync(type: string) {
    setSyncing(type);
    try {
      await api.post(`/admin/marketplace/trendyol/sync`, { type });
      fetchLogs();
    } catch (err) {
    ...
    <AdminShell title="Trendyol">
      ...
      <div className="bg-card-bg rounded-xl border border-border p-6 mb-6 max-w-2xl">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-xs text-text-secondary mb-1">Satici ID</label>
            <input
              className={inputCls}
              value={settings.seller_id}
              onChange={(e) => setSettings((s) => ({ ...s, seller_id: e.target.value }))}
              placeholder="Trendyol Satici ID"
            />
          </div>
      ...
      {!loading && !error && logs.length === 0 && (
        <div className="text-center py-12 text-text-secondary">
          Trendyol senkronizasyon logu bulunamadi.
        </div>
      )}
